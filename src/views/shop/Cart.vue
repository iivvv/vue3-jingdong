<template>
  <div class="mask" v-if="showCart && total > 0"
    ...
    <div class="product" v-if="showCart && total > 0">
      ...
      <!-- 购物车商品列表 -->
        ...
        <div class="product__item" v-for="item in productList" :key="item._id">
          <span class="product__item__checked iconfont"
...
const useCartEffect = (shopId) => {
  ...
  const { changeCartItemInfo, productList, total, price, allChecked } = useCommonCartEffect(shopId)
  // 取出store里的数据遍历要计算的属性
...
export default {
  ...
  setup () {
    ...
    const {
      total, price, allChecked, productList, changeCartItemInfo,
